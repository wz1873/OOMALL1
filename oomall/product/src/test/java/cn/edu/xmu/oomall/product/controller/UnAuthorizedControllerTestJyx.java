...
@Transactional
public class UnAuthorizedControllerTestJyx {
   ...
   @Test
    ...
    public void testGetProductProdGivenSuccess()throws Exception{
        ...
        Mockito.when(shopMapper.getTemplateById(10L,1L)).thenReturn(retTeplate);
   ...
   @Test
     ...
     */
    @Test
    public void testGetOnsaleGivenail2()throws Exception{
        Mockito.when(redisUtil.bfExist(Mockito.anyString(), (Long) Mockito.any())).thenReturn(true);
    ...
    public void testGetOnsaleGivenSuccess()throws Exception{
        ...
        Mockito.when(shopMapper.getShopById(10L)).thenReturn(retObj);
        Mockito.when(shopMapper.getTemplateById(10L,1L)).thenReturn(retTeplate);
        this.mockMvc.perform(MockMvcRequestBuilders.get("/onsales/{id}",1)
                        .header("authorization", adminToken)
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
                .andExpect(MockMvcResultMatchers.status().isOk());
