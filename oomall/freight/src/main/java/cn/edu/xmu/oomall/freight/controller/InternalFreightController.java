...
public class InternalFreightController {
    ...
    public ReturnObject createPackage(@PathVariable Long shopId,
        ...
        Express express = Express.builder().sendRegionId(expressVo.getSender().getRegionId())
                ...
                .shopLogisticsId(expressVo.getShopLogisticId()).build()
    ...
    @Audit(departName = "shops")
    @Transactional(propagation = Propagation.REQUIRED)
    public ReturnObject sendPackage(@PathVariable Long shopId, @PathVariable Long id, @LoginUser UserDto user) {
        this.expressService.sendExpress(shopId, id, user);
